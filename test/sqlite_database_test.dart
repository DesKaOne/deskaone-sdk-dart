...
void main() {
  ...
  test('create table, insert, query, queryOne, update, and delete', () async {
    ...
    expect(updated, 1);
    expect(
      (await db.queryOne('SELECT age FROM users WHERE name = ?', [
        'Alice',
      ]))?['age'],
      31,
    );

    final deleted = await db.execute('DELETE FROM users WHERE name = ?', [
      'Bob',
    ]);
    expect(deleted, 1);
  ...
  test('transaction commit persists changes', () async {
    ...
    expect(
      await db.query('SELECT * FROM users WHERE name = ?', ['Carol']),
      hasLength(1),
    );
  });
  ...
  test('transaction rollback discards changes', () async {
    ...
    expect(
      await db.query('SELECT * FROM users WHERE name = ?', ['Dave']),
      isEmpty,
    );
  });
}
